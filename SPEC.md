...
| ------------------------------------------------ | ---------------------- | --------------------------------------------------------------- |
| **Textbasiert** (CodeMirror 6)                   | Text + Dekorationen    | Deckungsgleich — der Puffer _ist_ Markdown                      |
...
| **renderRange(v)**     | **Initiale** Range von View `v`: `ownRange(scope)` bei `include: 'own'`, sonst `subtreeRange(scope)`. **`to` ist exklusiv:** die letzte editierbare Position ist `to - 1` (bzw. `to`, wenn `to` das Dokumentende ist). Ein Insert _bei_ `to` ist das erste Zeichen der nächsten Node. Der **lebende** Ausschnitt ist nicht `renderRange`, sondern `ScopeRange` (§ 3.6). |
...
**Host-Politik, keine Invariante**. Die Komponente muss sperren _können_; ob sie es tut,
...
| **L2**  | Getippte Markdown-Syntax wird nicht interpretiert, sondern maskiert geschrieben (`#`, `*`, `_`, `>`, `-`, Backtick, Backslash, `<`). **Ein Durchgang** über die ursprüngliche Einfügung. Der Maskierungs-Backslash wird nicht ein zweites Mal maskiert — sonst wird aus `#` zuerst `\#` und lebend `\\####`. In `wysiwyg` blendet die Darstellung den Maskierungs-Backslash aus (`\#` liest als `#`); der Puffer behält `\#`. Löschen des sichtbaren `#` entfernt den Backslash mit. Einfügungen der Kommandos C1–C3 sind ausgenommen (C4).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **L3**  | Mehrzeiliges Einfügen wird in einem Schritt maskiert und in einem Schritt zurückgenommen.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **L4**  | Überschriftentext bleibt in `wysiwyg` editierbar, unabhängig von `policy.structureEditingInWysiwyg` — **außer** der Scope-Überschrift, wenn `showNodeHeading: false` (SNH2): die ist ausgeblendet und nicht erreichbar. Zeilenumbruch (Enter) auf einer Überschriftenzeile fügt bei `'locked'` keine Zeile ein — das wäre Struktur, nicht Titeltext. Eine Leerzeile _zwischen_ Überschriften bleibt normale Prosa. Gilt bei `headingEditingInWysiwyg: 'inline'` (Default). Bei `'locked'` tritt LH1 an die Stelle der Titel-Editierbarkeit.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| **FM9** | In `hidden` ist die ausgeblendete **und gesperrte** Spanne der YAML-Zaun bis zur gebundenen Überschrift (inkl. Klebe-Leerzeilen nach dem schließenden Zaun). Die Leerzeile _nach_ der vorausgehenden Überschrift bleibt eine normale, sichtbare, editierbare Zeile.                                                                         |
...
| **W2** | Nicht sichtbare Anteile (übrige Attribute, Marker, Tags) nehmen an der Suche nicht teil. Ausnahme W7: der id-Wert bzw. der type-Token _ist_ das Label, nicht verstecktes Chrome.                                                                                                                                                                                                                                             |
...
| **C3** | `toggleWrapSelection(view, open, close?)` schaltet Wrap **word-like** um (`close` default = `open`). Leerer Caret expandiert zuerst auf das Wort in der Zeile (Buchstaben, Ziffern, `_`). Danach, in dieser Reihenfolge: (1) liegen die Marker unmittelbar außen um die Selektion → Marker entfernen; (2) beginnt und endet die Selektion selbst mit den Markern → Marker von der Selektion streifen; (3) sonst Marker-Vorkommen von `open`/`close` innerhalb der Selektion entfernen und die Selektion einmal außen wrappen (teilweise gewrappt → alles gewrappt, z. B. `**hello** was geht?` → `**hello was geht?**`). Ein zweiter Toggle auf einheitlich gewrapptem Text packt wieder aus. Die Selektion liegt danach auf dem Text, nicht auf den Markern. Bei einzeichigem `open`/`close` (z. B. `*`) zählen nur Marker-Grenzen, die nicht Teil einer längeren gleichen Run sind (`**` wird nicht als italic-Wrap gelesen).                                                         |
| **C4** | Die Einfügungen von C1–C3 sind autoritative Markdown-Syntax, keine getippte Meta, und umgehen die L2-Maskierung: `insertListPrefix` schreibt `- ` / `1. `, nicht `\- `; `toggleWrapSelection` schreibt `*` / `**`, nicht `\*` / `\*\*`; `setHeadingLevel` schreibt `# `, wo Struktur-Editieren erlaubt ist (`source`, `structureLocked: false`), nicht `\# `. Verschachtelte Wraps bauen echte Marker-Runs (`*Hallo*` → Bold → `***Hallo***`, ohne neue Backslashes). Getipptes/eingefügtes `*` / `-` / `#` behält L2 (IM3). Mechanik: die Kommando-Transaktion trägt `formatCommandAnnotation`; nur der L2-Maskierungsschritt erkennt sie und lässt die Einfügung roh durch — dieselbe gezielte Umgehung wie L5. Die vorgelagerten Guards greifen unverändert: `structureLocked` blockiert weiterhin jedes ATX-Marker-Schreiben (C1 ist dort ein No-op), und die Atom-/Marker-Overlap-Expansion (L1) läuft auch für C2/C3 — ein Kommando darf keinen bestehenden Marker-Run zerteilen. |
...
| **U14** | Die CM6-eigene History ist ein _Primitiv_, das die äußere Timeline ansteuert: bei einem Texteintrag ruft sie CM6-Undo (auf dem `SessionEditorState`, § 11.2), bei einem fremden Eintrag dessen `revert`, **ohne** die CM6-History anzufassen.                                                                                                                          |
...
| #        | Fall                                                                                                                                                                                                                                                                                                                                                                                            |
| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **T133** | `showNodeHeading: false` in `wysiwyg`: DOM zeigt den Titel des Scope-Knotens nicht, Kind-Titel bleiben; Document behält die ATX-Zeile. `setScope` aufs Kind blendet _dessen_ Heading aus. In `source` bleibt die Scope-Überschrift sichtbar. Suche nach dem Scope-Titel: kein Treffer in `wysiwyg`, Treffer in `source` (SNH2–SNH4/F4/F5). Default `true` ändert bestehende Fälle nicht (SNH1). |
